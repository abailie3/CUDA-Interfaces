...
v1.1: 4/XX/2017: Developing Python Interface
  ...
  - Changed matops and associated file names to neuralNet 
...
#include "JavaInputConversion.h"
#include "jInterface.h"
#include <random>
#include <thread>
#include "jInterface_JavaNNInterface.h"
using namespace std;
...
__device__ void LogisticOutputLayerPivotDevice(d_Mat2D d_previous, d_Mat2D d_current, 
	...
	if (row > 0 && row < max_row && col < max_col) {
		...
		if (row == 2 & col == 0) printf("Run: %d, Error Squared: %f, Actual: %f\n\n", run, err * err, act);
...
Reaches the end and pivots, comparing the output with the actual value in the
training set. Travels backwards through the recursion return path and
calculates the change to the node weights.
*/
...
void runBatch( int bSize, float alpha, ofstream &outFile,
  ...
  printf("\n=====================================================");
  printf("===== End Batch Run =====");
  printf("=====================================================\n");
}


JNIEXPORT void JNICALL Java_jInterface_JavaNNInterface_test
(JNIEnv *env, jobject obj, jstring who) {
	const char *test;
	jboolean iscopy;
	test = env->GetStringUTFChars(who, &iscopy);
	printf("C++ says %s \n", test);
}
...
*/
void printAndClear(Mat2D* &first, Mat2D* &inputs, Mat2D* &actual, bool del_ar = true) {
...
void runNeuralNet(LaySet lay, Mat2D* inputs, Mat2D* actual, 
                  int bSize, float alpha, ofstream &outFile, bool del_ar = true) {
  ...
  fflush(stdout);
  printAndClear(first, inputs, actual, del_ar);
  fflush(stdout);
...
void runNeuralNet(LaySet lay, Mat2D* inputs, Mat2D* actual,
	...
	ofstream outFile;
	outFile.open(oString);
	runNeuralNet(lay, inputs, actual, bSize, alpha, outFile);
	outFile.close();
}
...
JNIEXPORT void JNICALL Java_jInterface_JavaNNInterface_executeNeuralNet
(JNIEnv *env, jobject obj, jintArray npl, jint layers, jint bsize, jfloat alpha,
	jfloatArray in_arr, jfloatArray act_arr, jint train_rows, jint train_cols, jstring save) {
	printf("start \n");
	jboolean iscopy;
	ofstream outFile;
	outFile.open(env->GetStringUTFChars(save, &iscopy));
	printf("ofstream\n");
	int* lay_array = (int*)env->GetIntArrayElements(npl, &iscopy);
	printf("layarray\n");
	LaySet lay = JArrayToLayset(lay_array, layers);
	printf("layset\n");
	Mat2D* inputs = JArrayToInMat((float*)env->GetFloatArrayElements(in_arr, &iscopy), (int)train_rows, (int)train_cols);
	printf("inputs\n");
	Mat2D* actual = JArrayToInMat((float*)env->GetFloatArrayElements(act_arr, &iscopy), (int) train_rows, (int)train_cols);
	runNeuralNet(lay, inputs, actual, (int)bsize, (float)alpha, outFile, false);
	fflush(stdout);
	printf("runNN\n");
	fflush(stdout);
	printf("EXIT\n");
